...
class DebugTree extends AVLTree {
	...
	private void toString(final String prefix, final boolean isTail, final StringBuilder sb, final AVLNode node) {
		if (node != null) {
			if (node.right != null)
...
class AVLTreeTest {
	...
	void testRemove() {
		...
		if (PRINT)
			System.out.println(tree);
		final Object[] strings = IntStream.of(11, 3, 0, 18, 29).mapToObj(i -> String.valueOf(-i)).toArray();
	...
	@Test
	void testToArray() {
		final Object[] strings = IntStream.of(5, 1, 0, 3, 12, 7, 11, 29, 18).mapToObj(i -> String.valueOf(-i))
				.toArray();
		...
		for (int i = 0; i < N_TRIALS; i++) {
			...
			case TO_ARRAY:
				...
				assertEquals(size, in.length,
						String.format("Lengths differed: Expected: %d, but was: %d", size, in.length));
				break;
			default:
