...
class DebugTree extends MyBinarySearchTree {
	...
	private boolean isTreeStructure(final BinaryTreeNode n) {
		if (n != null) {
			...
			if (left != null && n.key.compareTo(left.key) < 0)
				return false;
			if (right != null && n.key.compareTo(right.key) > 0)
				return false;
...
class MyBinarySearchTreeTest {
	...
	/**
	 * Use Debug Tree in all tests and assert correctness in
	 * <tt>testRandomOperations</tt>
	 */
	private static final boolean USE_DEBUG_TREE = true;
	/**
	 * Print debug information in <tt>testRandomOperations</tt> Caution: Will slow
	 * down test considerably (~80 times slower on my machine)
	 */
	...
	void testRemove() {
		...
		if (PRINT)
			System.out.println(tree);
		final Object[] strings = IntStream.of(0, 3, 11, 18, 29).mapToObj(i -> String.valueOf(-i)).toArray();
		assertArrayEquals(strings, tree.toArrayInOrder(), "Structure of tree was violated");
	}

	@Test
	void testToArrayPostOrder() {
		final Object[] strings = IntStream.of(1, 3, 0, 11, 7, 18, 12, 29, 5).mapToObj(i -> String.valueOf(-i))
				.toArray();
		assertArrayEquals(strings, tree.toArrayPostOrder());
		assertArrayEquals(new String[0], new MyBinarySearchTree().toArrayPostOrder(),
				"Empty tree must produce empty array");
	}

	@Test
	void testToArrayInOrder() {
		final Object[] strings = IntStream.of(0, 1, 3, 5, 7, 11, 12, 18, 29).mapToObj(i -> String.valueOf(-i))
				.toArray();
		assertArrayEquals(strings, tree.toArrayInOrder());
		assertArrayEquals(new String[0], new MyBinarySearchTree().toArrayInOrder(),
				"Empty tree must produce empty array");
	}

	@Test
	void testToArrayPreOrder() {
		final Object[] strings = IntStream.of(5, 0, 3, 1, 29, 12, 7, 11, 18).mapToObj(i -> String.valueOf(-i))
				.toArray();
		assertArrayEquals(strings, tree.toArrayPreOrder());
		assertArrayEquals(new String[0], new MyBinarySearchTree().toArrayPreOrder(),
				"Empty tree must produce empty array");
	}
	...
	void testRandomOperations() {
		...
		for (int i = 0; i < N_TRIALS; i++) {
			...
			if (PRINT)
				System.out.println(String.format("[i:%d, c:%s, k:%d]", i, choice, k));
			if (PRINT_DEBUG)
				System.out.println(tree);
			...
			case TO_ARRAY:
				...
				if (pre.length != post.length || in.length != pre.length) {
					fail(String.format("Lengths of arrays differed: len(pre)=%d len(in)=%d len(post)=%d", pre.length,
