 ...
 * @author CyberFreak44
 */
public class TestQuadraticHashSetStefan {
	...
	void testNotContains() {
		...
		for (int i = 0; i < 20; i++) {
			if (i < 10)
				...
				assertFalse(hs.contains(i), "Element was wrongfully found in QuadraticHashSet");
		}
	}
	...
	void testRandomOperationsNoException() {
		...
		for (int i = 0; i < 10000; i++) {
			...
			switch (choice) {
			case 0:
				if (insertAsString((int) (Math.random() * 10)))
					size++;
				break;
			case 1:
				if (hs.remove((int) (Math.random() * 10)))
					size--;
				break;
			case 2:
	...
	void testToString() {
		...
		assertEquals("0 {-}, 1 {21}, 2 {-}, 3 {3}, 4 {23}, 5 {5}, 6 {45}, 7 {18}, 8 {58}, 9 {38}", hs.toString(),
	...
	void testRemoveWithMultiProbing() {
		...
		for (int i = 0; i < CAPACITY; i++) {
			if (i < 6)
				assertTrue(hs.remove(i * 10), "This key should be present.");
			else
				assertFalse(hs.remove(i * 10), "This key shouldn't be present.");
		}
	}
