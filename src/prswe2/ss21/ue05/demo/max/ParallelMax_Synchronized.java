...
public class ParallelMax_Synchronized {
	
	private ExecutorService executor = Executors.newFixedThreadPool(10); 

	private final long[] numbers; 
	private final int parallelism; 
	private long max; // mutable shared state
	private CountDownLatch endLatch; 
	
	ParallelMax_Synchronized(long[] numbers, int parallelism) {
	...
	private class MaxRunnable implements Runnable {
		...
		public void run() {
			for (int i = from; i < to; i++) {
				// Different possibilities for synchronization 
				synchronized (numbers) {
					if (numbers[i] >= max) {
						max = numbers[i]; 
					}
